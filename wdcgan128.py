...
class Generator(nn.Module):
    def __init__(self, params):
        ...
        self.getnorm=params.get("Gnorm","instance")
        def get_norm_layer(channels, H, W):
            if self.getnorm == 'instance':
                return nn.InstanceNorm2d(channels, affine=True)
            elif self.getnorm == 'layer':
                return nn.LayerNorm([channels, H, W])
            elif self.getnorm == 'batch':
